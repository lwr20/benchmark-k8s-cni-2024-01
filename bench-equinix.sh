...
function test_setup {
    ...
    log start RKE2
    ./equinix.sh rke2-up > /dev/null 2>&1
    log end RKE2
    ...
    if [ "${BENCHID:0:3}" = "st_" ]
    then
        ./equinix.sh cni ${BENCHID:3} > /dev/null 2>&1
    else
        ./equinix.sh cni ${BENCHID} > /dev/null 2>&1
    fi
...
function test_cleanup {
    ...
    log start
    ./equinix.sh cleanup > /dev/null 2>&1
    sleep 10
    log end
}
...
function bench_baremetal {
    BENCHID="$1"
    EXEC_CMD="./equinix.sh ssh"

    CMDA1="./equinix.sh ssh a1"
    CMDA2="./equinix.sh ssh a2"
    CMDA3="./equinix.sh ssh a3"

    DIRECT_A1="$(./equinix.sh getip a1)"
    DIRECT_A2="$(./equinix.sh getip a2)"
    DIRECT_A3="$(./equinix.sh getip a3)"
